...
class AgentAdminAppGenerator {
    constructor(adminID, appName, configs, newPackage, id) {
        this.id = id;
        this.adminID = adminID;
        this.appName = appName + " Admin";
        this.configs = configs;
        this.iconPath = path.join(__dirname, "../../data/uploads/" + id + "/app_icon.png");
        this.projectPath = path.join(__dirname, "../../data/temp/" + id + "/agent-admin");
        this.mainProjectPath = path.join(__dirname, "../../data/main/admin-project.zip");
    ...
    async unzipProject() {
        ...
        await new Promise((resolve, reject) => {
            fs.createReadStream(this.mainProjectPath)
                .pipe(unzipper.Parse())
                .on('entry', async (entry) => {
                    const filePath = path.join(this.projectPath, entry.path);
    ...
    renamePackage() {
        const oldPackage = "com.pegalite.neotronadmin";
    ...
    updateConfigs() {
        ...
        const utilsPath = path.join(this.projectPath, "app", "src", "main", "java", this.newPackage.replaceAll(".", "/"), "functions", "Utils", "Utils.java");
        let utilsContent = fs.readFileSync(utilsPath, "utf8");
        const newIDAttribute = `ADMIN_CONFIGS = "${this.configs}"`;
...
module.exports = AgentAdminAppGenerator